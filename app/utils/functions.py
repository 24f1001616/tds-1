...
async def calculate_prettier_sha256(filename: str) -> str:
    ...
    try:
        ...
        print(f"Starting SHA256 calculation for file: {filename}")
        ...
        print(f"File {filename} found. Proceeding with Prettier formatting.")

        # Find npx executable path
        npx_path = shutil.which("npx")
        if not npx_path:
            print("npx not found in system PATH. Checking common Windows locations...")
        ...
        if not npx_path:
            print("npx executable not found, using direct file hash calculation.")
            # If npx is not found, read the file and calculate hash directly
            with open(filename, "rb") as f:
                content = f.read()
                hash_obj = hashlib.sha256(content)
                hash_value = hash_obj.hexdigest()
            print(f"SHA256 hash calculated (direct): {hash_value}")
            return f"{hash_value} *-"

        # On Windows, we need to use shell=True and the full command
        print(f"Running Prettier command: {npx_path} -y prettier@3.4.2 {filename}")
        ...
        try:
            ...
            print("after prettier output")
            # Calculate hash directly from the prettier output
            hash_obj = hashlib.sha256(prettier_output.encode("utf-8"))
            print("hash object:",hash_obj)
            hash_value = hash_obj.hexdigest()
            print("hash value:",hash_value)

            print(f"SHA256 hash after Prettier formatting: {hash_value}")
            return f"{hash_value} *-"

        except subprocess.CalledProcessError as e:
            print(f"Error running Prettier: {e.output}")
